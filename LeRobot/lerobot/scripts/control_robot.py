...
import json
YOLO_JSON_PATH = "object_positions.json"       # YOLO 检测脚本输出位置
EXTRA_FEATURES = {                             # 追加到 dataset.features
    "yolo_json": {
        "dtype": "string",
        "shape": (1,),
        "names": None,
...
def record(
    ...
    if cfg.resume:
        ...
        if "yolo_json" not in dataset.features:
            dataset.meta.features.update(EXTRA_FEATURES)
        ...
        dataset.meta.features.update(EXTRA_FEATURES)
        dataset.episode_buffer = dataset.create_episode_buffer()
    def _record_episode_with_yolo(**kwargs):
            ds = kwargs["dataset"]
            orig_add = ds.add_frame

            def add_frame(frame_dict):
                try:
                    with open(YOLO_JSON_PATH, "r", encoding="utf-8") as jf:
                        frame_dict["yolo_json"] = json.dumps(json.load(jf))
                except FileNotFoundError:
                    frame_dict["yolo_json"] = ""          # 没检测到就存空串
                orig_add(frame_dict)

            ds.add_frame = add_frame
            try:
                return record_episode(**kwargs)          # 调用原实现
            finally:
                ds.add_frame = orig_add                  # 复原
